...
DOCKER_IMG_NAME="bulletplus/qinglong"
JD_PATH=""
SHELL_FOLDER=$(pwd)
CONTAINER_NAME=""
TAG=""
...
# 拉取版本
echo -n -e "\e[33m二、请输入要安装的版本，如安装2.10.2版（推荐）直接回车:\e[0m " 
read version
	if [ ! -n "${version}" ];then
    TAG=2.10.2
else
...
if [ "$port" = "2" ]; then
    ...
    docker exec -it $CONTAINER_NAME bash -c "ql check"
...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $CONFIG_PATH/auth.json)" != 0 ]; then
        log "7.开始安装或重装 Ninja"
        if [ "$INSTALL_NINJA" = true ]; then
            docker exec -it $CONTAINER_NAME bash -c "cd /ql;ps -ef|grep ninja|grep -v grep|awk '{print $1}'|xargs kill -9;rm -rf /ql/ninja;git clone https://ghproxy.com/https://github.com/Bulletgod/Waikiki_ninja.git /ql/ninja;cd /ql/ninja/backend;pnpm install;cp .env.example .env;cp sendNotify.js /ql/scripts/sendNotify.js;sed -i \"s/ALLOW_NUM=40/ALLOW_NUM=100/\" /ql/ninja/backend/.env;pm2 start"
...
read -p "安装脚本依赖（跳过为 0，安装为 1，回车等同 1）请输入：" Dependency

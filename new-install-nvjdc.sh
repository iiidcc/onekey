...
cat > /root/nvjdc/Config/Config.json << EOF
{
  ...
  "WP_APP_TOKEN": "${WxPushertoken}",
  "MainWP_UID": "${WxMainWP_UID}",
  // ======================================= pushplus 通知设置区域 ===========================================
...
read -p "请输入自动滑块次数 直接回车默认5次后手动滑块 输入0为默认手动滑块: " AutoCaptcha && printf "\n"
    ...
    sed -i "14a \        \"AutoCaptchaCount\": \"5\"," /root/nvjdc/Config/Config.json
else
    sed -i "14a \        \"AutoCaptchaCount\": \"${AutoCaptcha}\"," /root/nvjdc/Config/Config.json
fi
...
menu() {
  echo -e "\
${green}0.${plain} 退出脚本
${green}1.${plain} 安装nvjdc2.2
${green}2.${plain} 升级nvjdc2.2

...
git clone git clone https://ghproxy.com/https://github.com/Bulletgod/nvjdcdocker.git /root/nolanjdc
...
mkdir /root/nolanjdc/Config && cd /root/nolanjdc/Config
wget -O Config.json   https://raw.githubusercontent.com/Bulletgod/nvjdcdocker/main/Config/Config.json
...
read -p "请输入自动滑块次数 直接回车默认5次后手动滑块 输入0为默认手动滑块: " AutoCaptcha && printf "\n"
    ...
    sed -i "14a \        \"AutoCaptchaCount\": \"5\"," /root/nolanjdc/Config/Config.json
else
    sed -i "14a \        \"AutoCaptchaCount\": \"${AutoCaptcha}\"," /root/nolanjdc/Config/Config.json
fi
...
docker pull Bulletplus/nvjdc:${version1}
...
-it --privileged=true  bulletplus/nvjdc:${version1}
...
baseip=$(curl -s ipip.ooo)  > /dev/null
docker rm -f nvjdc
docker pull Bulletplus/nvjdc
docker run   --name nvjdc -p ${portinfo}:80 -d  -v  "$(pwd)":/app \
-v /etc/localtime:/etc/localtime:ro \
-it --privileged=true  Bulletplus/nvjdc
docker update --restart=always nvjdc
...
menu() {
  echo -e "\
${green}0.${plain} 退出脚本
${green}1.${plain} 安装nvjdc2.4
${green}2.${plain} 升级nvjdc2.4
